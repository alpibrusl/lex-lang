...
| Agent-native version control | tier-1 ✅ — `lex branch` + `lex store-merge` with structured JSON conflicts ; **`lex log` ✅** (per-branch merge journal) ; **tier-2 ✅** — operation log as source of truth: `lex publish` emits typed ops, `lex op show` / `lex op log`, `lex blame` causal history (#129 done) ; write-time type-check gate (#130) next ; distributed sync + body-level merge deferred |
