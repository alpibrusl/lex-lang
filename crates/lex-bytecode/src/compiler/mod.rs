...
use constpool::ConstPool;
use free_vars::free_vars;
use liveness::apply_last_load_takes;
use lowering::{apply_arena_lowering, apply_escape_lowering};
use peephole::{
    apply_peephole, apply_peephole_slice2, apply_peephole_slice3, apply_peephole_slice4,
    apply_peephole_slice5, apply_peephole_slice6, apply_peephole_slice7, apply_peephole_slice9,
};
...
pub fn compile_program(stages: &[a::Stage]) -> Program {
    ...
    // #774: move-out loads. A `LoadLocal` whose slot is dead on every
    // path after it becomes `TakeLocal`. Runs after the peepholes so
    // every fusion still fires. Escape hatch: `LEX_NO_TAKE_LOCALS=1`,
    // for A/B benches, mirroring the two lowering flags above.
    if std::env::var_os("LEX_NO_TAKE_LOCALS").is_none() {
        for f in p.functions.iter_mut() {
            apply_last_load_takes(&mut f.code);
        }
    }
