...
//!
//! Every `publish` below captures files by default (#1007 PR 4) and every
//! `op push` syncs their blobs (#1007 PR 5) -- this test doesn't opt out
//! with `--no-files`, which is itself part of what it's proving: a directory
//! publish, push, release and cross-hub install chain all keep working end
//! to end once real files (a captured `lex.toml`/`lex.lock`, at minimum)
//! are riding along with every head.
