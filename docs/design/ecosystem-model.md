...
| loom ↔ lex-os | **Designed, not wired.** `lex-os-manifest` is a declared dependency; `src/manifests.lex` already generates a role-scoped `Grant` per sprint phase (e.g. Design: read-only/no-exec, Build: read-write/sandboxed-exec, QA: sandboxed-exec for tests); `docs/design/lex-os-isolation.md` has the full per-role table and a rollout plan. No code path yet from a generated grant to an actual `lex-os capsule install`. |
